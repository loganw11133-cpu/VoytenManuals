/* decoder-pdf.js — Shared PDF export for all Voyten decoder tools
   ...
   Auto-injects a "Download PDF" button next to the Copy button.
...
(function () {
  ...
  function injectCSS() {
    ...
    s.textContent =
      '.btn-pdf{padding:8px 20px;border:2px solid #dc2626;border-radius:8px;background:#fff;color:#dc2626;' +
      'font-weight:600;font-size:0.8rem;cursor:pointer;transition:all 0.2s;margin-left:8px;touch-action:manipulation;}' +
  ...
  function injectButton() {
    /* Don't duplicate */
    if (document.querySelector('.btn-pdf')) return;

    /* Find the copy button — could be inside #results or in #copyRow */
    var copyBtn = document.querySelector('#copyRow .btn-copy') ||
                  document.querySelector('#results .btn-copy') ||
    ...
    btn.onclick = function () { window.downloadDecoderPDF(); };
    copyBtn.insertAdjacentElement('afterend', btn);
  }

  /* Watch for results to appear, then inject button */
  var resultsEl = document.getElementById('results');
  if (resultsEl) {
    new MutationObserver(function () {
      if (resultsEl.children.length > 0) injectButton();
    }).observe(resultsEl, { childList: true });
  }
  /* Also watch #copyRow for RL decoder pattern */
  var copyRow = document.getElementById('copyRow');
  if (copyRow) {
    new MutationObserver(function () {
      if (copyRow.style.display !== 'none') injectButton();
    }).observe(copyRow, { attributes: true, attributeFilter: ['style'] });
  }

  /* ── PDF Builder ── */

  window.downloadDecoderPDF = function () {
    ...
    var cat = getCatalogNumber();
    var name = getDecoderName();
